                ...
                <div class="form-group row">
                    <?php foreach($dental_history as $mh){ ?>
                        ...
                        <div class="col-sm-10">
                            <div class="form-check-inline">
                                <label class="form-check-label">
                                <input type="checkbox" class="form-check-input dental_history" name="dental_history[]" id="dental_history_<?php echo $mh->id;?>" value="<?php echo $mh->id;?>" >
...
<script>
  $(document).ready(function(){
    ...
    $("#search").click(function(){
        ...
        // $(".").select2("val", "");
        $(".dental_history").prop('checked',false);
        $(".error").remove();
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data.patient_details.length<=0){
                    ...
                    if(data.dental_history!=null){
                        var res = data.dental_history.dental_history_id.split(',');
                        $.each(res,function(key,val){
                            $("#dental_history_"+val).prop('checked',true);
                        });
                    }
...
//     $(".error").remove();
    ...
    $('.dental_history:checked').each(function(s){
        //   val[s] = $(this).val();
        dental_history_id[s]=$(this).val();
    });
    
    $.ajax({
        url: "<?php echo base_url('ClinicalExaminationController/store_dental_history_details');?>",
        data: ({patient_id:patient_id,dental_history_id:dental_history_id}),
        dataType: 'json', 
        type: 'post',
        success: function(data) {
