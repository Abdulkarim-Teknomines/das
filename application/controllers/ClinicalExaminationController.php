...
class ClinicalExaminationController extends MY_Controller {
  ...
  public function store_clinical_examinator_details(){
    ...
    $data = array(
      'patient_id'=>$this->input->post('patient_id'),
      'clinical_examinator'=>$trt_plan_id,
      'clinic_user_id'=>$this->admin_session->id,
      'clinic_id'=>$this->admin_session->clinic_id
    );
    $ins_data = $this->Patient_model->insert_data($data,'da_patient_clinical_examinator');
    if($ins_data!=false){
      $data=array('status'=>'success','message'=>'Clinical Examinator Added Successfully');
      echo json_encode($data);
    }
  }
  public function store_investigation_details(){
    ...
    $data = array(
      'patient_id'=>$this->input->post('patient_id'),
      'investigation'=>$trt_plan_id,
      'clinic_user_id'=>$this->admin_session->id,
      'clinic_id'=>$this->admin_session->clinic_id
    );
    $ins_data = $this->Patient_model->insert_data($data,'da_patient_investigation');
    if($ins_data!=false){
      $data=array('status'=>'success','message'=>'Clinical Examinator Added Successfully');
      echo json_encode($data);
    }
  }
  public function store_diagnosis_details(){
    $trt_plan_id = '';
    $this->db->where('patient_id', $this->input->post('patient_id'));
    $this->db->delete('da_patient_diagnosis');

    if(!empty($this->input->post('diagnosis'))){
      $trt_plan_id = implode(',',$this->input->post('diagnosis'));
    }
    
    $data = array(
      'patient_id'=>$this->input->post('patient_id'),
      'diagnosis'=>$trt_plan_id,
      'clinic_user_id'=>$this->admin_session->id,
      'clinic_id'=>$this->admin_session->clinic_id
    );
    $ins_data = $this->Patient_model->insert_data($data,'da_patient_diagnosis');
    if($ins_data!=false){
      $data=array('status'=>'success','message'=>'Diagnosis Added Successfully');
      echo json_encode($data);
    }
  }
}
